...
Terrain::Terrain()
    : GameObject(L"Landscape/Terrain.hlsl"), width(TERRAIN_WIDTH), height(TERRAIN_HEIGHT)
{    
    material->SetDiffuseMap(L"Textures/Landscape/Grass.png");
    secondMap = Texture::Add(L"Textures/Landscape/Dirt.png");
    thirdMap = Texture::Add(L"Textures/Landscape/Stones.png");
    heightMap = Texture::Add(L"Textures/HeightMaps/TavernHeightMap.png");
    alphaMap = Texture::Add(L"Textures/AlphaMaps/TavernAlphaMap.png");
...
void Terrain::RenderInstanced(UINT instanceCount)
{
    material->Set();
...
void Terrain::MakeMesh()
{
    ...
    for (UINT z = 0; z < height; z++)
    {
        ...
        {
            VertexType vertex;
            vertex.pos = { TERRAIN_STARTPOS.x + (float)x, 0.0f, TERRAIN_STARTPOS.z +(float)(height - z - 1)};
            vertex.uv.x = x / (float)(width - 1);
            vertex.uv.y = z / (float)(height - 1);

            UINT index = width * z + x;
            vertex.pos.y = ( 1 - pixels[index].x ) * MAX_HEIGHT;
